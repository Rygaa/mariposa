<script lang="ts">
  ...
  async function loadOrders() {
    ...
    try {
      ...
      const result = await trpc.listOrders.query(filters);
      if (result.success) {
...
<Page>
  <Card>
    <CardHeader>
      ...
      <CreateOrderModal onOrderCreated={loadOrders} />
    </CardHeader>
      ...
      <DataDisplayer {isLoading} isEmpty={filteredOrders.length === 0}>
        ...
        >
          {#each filteredOrders as order (order.id)}
            <Order {order} onOrderUpdated={loadOrders} />
          {/each}
        </div>
      </DataDisplayer>
    </CardContent>
