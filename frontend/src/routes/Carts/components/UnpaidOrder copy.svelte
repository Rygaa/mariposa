  ...
  <CardContent class="p-4 flex flex-col">
    ...
    <div class="space-y-2 mb-4">
      {#if sortedMenuItemOrders && sortedMenuItemOrders.length > 0}
        {#each sortedMenuItemOrders as menuItemOrder (menuItemOrder.id)}
          {@const change = menuItemChanges.get(menuItemOrder.id)}
          {@const isDeleted = change?.toDelete ?? false}
          {@const displayQuantity = change?.quantity ?? menuItemOrder.quantity}
          {@const mainItemTotal =
            (change?.price ?? menuItemOrder.price) * displayQuantity}
          {@const supplementsTotal = (menuItemOrder.supplements || []).reduce(
            (sum: number, supp: any) => {
                ...
                >
                  ...
                  {#if menuItemOrder.menuItem?.subName}
                    <span class="font-normal"
                      >({menuItemOrder.menuItem.subName})</span
                    >
                  {/if}
                </span>
              ...
              {#if menuItemOrder.supplements && menuItemOrder.supplements.length > 0}
                <div class="mt-1 ml-4 text-xs text-gray-500">
                  {#each menuItemOrder.supplements as supplement}
                    ...
                    {@const suppQuantity =
                      suppChange?.quantity ?? supplement.quantity}
                    <div
                      class={suppChange?.toDelete
                    ...
                    >
                      ...
                      × {suppQuantity}
                      <span class="ml-1"
                        >({formatCurrency(supplement.price)})</span
                      >
                    </div>
                  {/each}
                </div>
            ...
            <div class="flex flex-col">
              <div class="flex items-center mb-2 gap-2 justify-end">
                ...
                <Button
                  ...
                  onclick={() =>
                    isDeleted
                      ? undoDelete(menuItemOrder.id)
                      : markForDeletion(menuItemOrder.id)}
                  disabled={isLocked || isSubmitting}
                  iconOnly={true}
                  iconName={isDeleted ? "reset" : "trash"}
                />
              </div>
              {#if !isDeleted}
