<script lang="ts">
  ...
  async function loadPrices() {
    ...
    try {
      ...
      if (result.success) {
        sellingPrices = result.itemPrices;
      }
    } catch (err: any) {
      error = err.message || "Failed to load selling prices";
    }
    isLoading = false;
  }
...
>
  <DialogContent class="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
    <DialogHeader>
      <DialogTitle>Selling Prices History - {menuItemName}</DialogTitle>
      <DialogDescription>
        View all selling prices for this item
      </DialogDescription>
    </DialogHeader>
    ...
    <div class="flex-1 overflow-y-auto p-4">
      {#if isLoading}
        <div class="flex items-center justify-center py-12">
          <div class="text-gray-500">Loading selling prices...</div>
        </div>
      {:else if sellingPrices.length === 0}
        <div class="flex flex-col items-center justify-center py-12">
          <Icon iconName="money" size="12" class="fill-gray-300 mb-4" />
          <p class="text-gray-500">No selling prices found</p>
        </div>
      {:else}
        <div class="space-y-3">
          {#each sellingPrices as price}
            <div
