<script lang="ts">
  ...
  let {
    ...
    placeholderText = "?",
    size = "6", // Tailwind size (w-4, h-4, etc.)
  }: any = $props();
  ...
  function addAttributesToSvg(content: string, cssClass: string, svgTitle: string, hidden: boolean): string {
    ...
    if (/width=/.test(attributes)) {
      attributes = attributes.replace(/width\s*=\s*["'][^"']*["']/i, `width="100%"`);
    } else {
      attributes += ` width="100%"`;
    }
    attributes = attributes.replace(/height\s*=\s*["'][^"']*["']/i, `height="100%"`);
...
{:else if hasError}
  ...
  {/if}
{:else if svgContent}
  <div class="inline-block w-{size} h-{size}">
    {@html svgContent}
  </div>
{/if}
