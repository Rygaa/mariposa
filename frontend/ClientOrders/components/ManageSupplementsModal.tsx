...
const MenuItemToUpdateSupplements = ({
  menuItemToUpdateSupplements,
  index,
  order,
  menuItem,
}: {
  menuItemToUpdateSupplements: any;
  index: number;
  order: any;
  menuItem: any;
}) => {
  ...
  const handleAdd = async (menuItemOrderId: string, supplementId: string) => {
    ...
    const orderItem = order.MenuItemOrderContainer.find((item: any) => item.id === menuItemOrderId);
    if (orderItem) {
      orderItem.selectedSupplements ||= [];
      const supplement = menuItem.supplements?.find((s: any) => s.supplement.id === supplementId)?.supplement;
    ...
    <div className="border rounded-lg p-4 bg-gray-50">
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-sm text-gray-600">
          {menuItemToUpdateSupplements.selectedSupplements?.length || 0} supplément(s)
        </span>
      </div>

      {menuItem.supplements?.length ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {menuItem.supplements.map((supp: any) => {
            const id = supp.supplement.id;
            const key = `${menuItemToUpdateSupplements.id}-${id}`;
            const isAdded = menuItemToUpdateSupplements.selectedSupplements?.some(
            ...
            return (
              <SupplementRow
                ...
                onAdd={() => handleAdd(menuItemToUpdateSupplements.id, id)}
                onRemove={() => handleRemove(menuItemToUpdateSupplements.id, id)}
              />
            );
          })}
        </div>
...
const ManageSupplementsModal = (props: SelectTableModalProps) => {
  return (
    <Modal isOpen={props.isOpen} setIsOpen={props.setIsOpen} width="100%" height="100%">
      <div className="w-full h-full flex flex-col">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto">
          {props.menuItemsOrder ? (
            props.menuItemsOrder.map((menuItemToUpdateSupplements: any, index: number) => (
              <MenuItemToUpdateSupplements
                index={index}
                order={props.order}
                menuItem={props.menuItem}
                menuItemToUpdateSupplements={menuItemToUpdateSupplements}
                key={menuItemToUpdateSupplements.id}
              />
            )) // patienter svp
          ) : (
            <div className="text-center py-4">
              <p className="text-gray-500 text-sm">
                Patientez s'il vous plaît, chargement des suppléments...
              </p>
            </div>
          )}
        </div>

        {/* Sticky Fermer Button at bottom of modal */}
        <div className="sticky bottom-0 bg-white border-t border-gray-200 pt-4 mt-6">
          <div className="flex justify-end">
            <button
              onClick={() => props.setIsOpen(false)}
              className="px-6 py-3 bg-white border-1 border-gray-300 text-gray-700 rounded-l hover:shadow-xl hover:bg-gray-50 transition-all duration-200 font-medium"
            >
              Fermer
            </button>
          </div>
...
export default ManageSupplementsModal;
